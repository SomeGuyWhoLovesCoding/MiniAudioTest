...
HL_PRIM void HL_NAME(set_playback_rate)(float value) {
	if (exists == 0) return;
	if (value == playbackRate) return; // No change
...
HL_PRIM void HL_NAME(start_hl)(_NO_ARG) {
	if (exists == 0) return;
	if (MIXER_STATE == 3) {
		seekToPCMFrame(0);
	}
...
HL_PRIM void HL_NAME(stop_hl)(_NO_ARG) {
	if (exists == 0) return;
	ma_device_stop(&device);
	MIXER_STATE = 2;
}

HL_PRIM bool HL_NAME(stopped_hl)(_NO_ARG) {
	return MIXER_STATE == 3 ? 1 : 0;
}

HL_PRIM void HL_NAME(destroy_hl)(_NO_ARG) {
	if (exists == 0) return;
	exists = 0;
	ma_device_uninit(&device);
